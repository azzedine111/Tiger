...
%type <ast::exps_type*>       ss_exps
%type <ast::CallExp*>         callexp
%type <ast::FieldInit*>       field
%type <ast::fieldinits_type*> fields
%type <ast::fieldinits_type*> ss_fields
...
%type <ast::VarDec*>          varfield
%type <ast::VarChunk*>        varfields varfields.1
...
// We want the latter.
%precedence CHUNKS
%precedence TYPE FUNCTION PRIMITIVE
...
exp:
  ...
  | exp AND exp                 { Tweast in; in << $3 << " <> 0"; auto res = tp.parse(in);  $$ = tp.td_.make_IfExp(@$, $1, res, tp.td_.make_IntExp(@$, 0)); }
  ...
  | MINUS exp                   { std::cout << "- minus" << std::endl;         }
...
| "import" STRING chunks        { $$ = tp.parse_import($2, @$); $$->ast::ChunkList::splice_back(*$3); }
| CHUNKS "(" INT ")" chunks { $$ = metavar<ast::ChunkList>(tp, $3); $$->ast::ChunkList::splice_back(*$5); }
;
...
callexp:
  ID "(" exps ")"            { $$ = tp.td_.make_CallExp(@$, $1, $3);}
...
ss_exps:
  exp                      { $$ = tp.td_.make_exps_type(); $$->push_back($1); }
| ss_exps ";" exp          { $1->push_back($3); $$ = $1; }
;
...
fields:
 fields "," field           { $1->push_back($3); $$ = $1; }
 | field                      { $$ = tp.td_.make_fieldinits_type(); $$->push_back($1);}
 ;
...
| CAST "(" exp "," ty ")"       { $$ = tp.td_.make_CastExp(@$, $3, $5);}
| callexp                       { $$ = $1; }
| EXP "(" INT ")"               { $$ = metavar<ast::Exp>(tp, $3); }
| ID "[" exp "]" OF exp         { $$ = tp.td_.make_ArrayExp(@$, tp.td_.make_NameTy(@$, $1), $3, $6);}
| NAMETY "(" INT ")"  "[" exp "]" OF exp {}
 //| fields                        { $$ = $1;}
| typeid "{" "}"                { $$ = tp.td_.make_RecordExp(@$, $1, nullptr);}
| typeid ss_fields              { $$ = tp.td_.make_RecordExp(@$, $1, $2);}
...
fundec:
  "function" ID "(" varfields ")"  "=" exp              { $$ = tp.td_.make_FunctionDec(@$, $2, $4, nullptr, $7); }
| "function" ID "(" varfields ")" ":" typeid "=" exp    { $$ = tp.td_.make_FunctionDec(@$, $2, $4, $7, $9); }
| "primitive" ID "(" varfields ")" ":" typeid           { $$ = tp.td_.make_FunctionDec(@$, $2, $4, $7, nullptr); }
| "primitive" ID "(" varfields ")"                      { $$ = tp.td_.make_FunctionDec(@$, $2, $4, nullptr, nullptr); }
;
...
varfields.1:
  varfields.1 "," varfield  { $$ = $1; $$->emplace_back(*$3); }
| varfield                  { $$ = tp.td_.make_VarChunk(@$);
                                    $$->emplace_back(*$1);}
;
